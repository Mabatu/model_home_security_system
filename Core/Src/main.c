...
int main(void)
{
  ...
  Lcd_clear(&lcd);
  Lcd_cursor(&lcd, 0, 2);
  Lcd_string(&lcd, "HOME SECURITY");
  Lcd_cursor(&lcd, 1, 5);
  Lcd_string(&lcd, "SYSTEM");
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if(HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_7) == GPIO_PIN_SET){
		  ...
		  Lcd_string(&lcd, "SYSTEM");
		  Lcd_cursor(&lcd, 1, 3);
		  Lcd_string(&lcd, "DEACTIVATED");
		  //A one second delay for the user to read the message
		  HAL_Delay(1000);
		  //Default state
		  Lcd_clear(&lcd);
		  Lcd_cursor(&lcd, 0, 2);
